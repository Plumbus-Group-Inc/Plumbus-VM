...
TEST(Interpreter, SquareEquation) {
  ...
  std::vector<Instr> instrs{
      ...
      Instr{.opcode = OPCODE_LOAD_IMM_F, .rd = 0x9, .rs1 = 0x0, .rs2 = 0x0, .immf = 1e-6F},  // loadImmF 10-e6, r9         # r9 <- 1e-6
      Instr{.opcode = OPCODE_ALU_UNARY,  .rd = 0xa, .rs1 = 0x8, .rs2 = 0x0, .op = OP_ABS_F}, // absF ra, r8                # ra <- |D|
      Instr{.opcode = OPCODE_BLESS_F,    .rd = 0x0, .rs1 = 0xa, .rs2 = 0x9, .immi = 0xdead}, // branchLessF 0xdead, ra, r9 # one_root

      Instr{.opcode = OPCODE_LOAD_IMM_F, .rd = 0xb, .rs1 = 0x0, .rs2 = 0x0, .immf = 0.0},    // loadImmF 0.0, rb           # rb <- 0.0
      Instr{.opcode = OPCODE_BLESS_F, .   rd = 0x0, .rs1 = 0x8, .rs2 = 0x9, .immi = 0xdead}, // branchLessF 0xdead, r8, r9 # no_roots
      ...
      /* one_root */
      Instr{.opcode = OPCODE_LOAD_IMM_F, .rd = 0x15, .rs1 = 0x00, .rs2 = 0x00, .immf = 2.0},      // loadImmF 2.0, r15   # r15 <- 2.0
      Instr{.opcode = OPCODE_ALU_BINARY, .rd = 0x16, .rs1 = 0x0b, .rs2 = 0x02, .op = OP_SUB_F},   // subF r16, rb, r2    # r16 <- -b
      Instr{.opcode = OPCODE_ALU_BINARY, .rd = 0x17, .rs1 = 0x16, .rs2 = 0x15, .op = OP_DIV_F},   // divF r17, r16, r15  # r17 <- - b / 2
      Instr{.opcode = OPCODE_ALU_BINARY, .rd = 0x18, .rs1 = 0x17, .rs2 = 0x01, .op = OP_DIV_F},   // divF r18, r17, r1   # r18 <- x <- - b / 2a
      Instr{.opcode = OPCODE_IO,         .rd = 0x00, .rs1 = 0x18, .rs2 = 0x00, .op = OP_WRITE_F}, // writeF r8           # print x
