...
TEST(Interpreter, SquareEquation) {
  ...
  std::vector<Instr> instrs{
    ...
    Instr{.opType = eUNARY, .opID = eUNARY_READ, .instrVar = InstrUNARY::Builder().ttypeid(kFloat).build()},
    ...
    Instr{.opType = eBINARY, .opID = eBINARY_MUL, .instrVar = InstrBINARY::Builder().ttypeid(kFloat).regid1(2).regid2(2).build()},
    ...
    Instr{.opType = eBINARY, .opID = eBINARY_MUL, .instrVar = InstrBINARY::Builder().ttypeid(kFloat).regid1(1).regid2(3).build()},
    ...
    Instr{.opType = eBINARY, .opID = eBINARY_MUL, .instrVar = InstrBINARY::Builder().ttypeid(kFloat).regid1(4).regid2(6).build()},
    Instr{.opType = eREG, .opID = eREG_MOV, .instrVar = InstrREG::Builder().regid(7).build()},

    // Instr{.opcode = OPCODE_ALU_BINARY, .rd = 0x8, .rs1 = 0x5, .rs2 = 0x7, .op = OP_SUB_F}, // subF r8, r5, r7 # r8 <- D
    Instr{.opType = eBINARY, .opID = eBINARY_SUB, .instrVar = InstrBINARY::Builder().ttypeid(kFloat).regid1(5).regid2(7).build()},
